    ...
    <form>
      ...
      <div class="flex items-center">
        <div class="flex-none w-64 pr-8">
          <img
            src="../assets/images/estage-logo.png"
        ...
        <div class="flex-auto">
          <div v-if="!isEditOrCreate" class="presentation-title-border">
            <div class="ml-4">
              ...
              <div class="text-gray-600 text-6xl mb-10">
                {{ enterprise.name }}
              </div>
              <div
                v-if="enterprise.activitySector"
                class="text-gray-600 bg-white py-2 px-4 inline text-2xl"
              >
                {{ enterprise.activitySector.value }}
              </div>
            </div>
          </div>
          <div v-else class="mb-20">
            <div class="grid grid-cols-12 mb-3">
              <label
                class="col-span-3 text-gray-600 text-2xl mr-4"
                for="enterpriseName"
                >Nom de l'entreprise:</label
              >
              <input
                class="col-span-9 border border-gray-400 rounded py-3 px-3 text-gray-600 text-2xl leading-tight focus:border-gray-800 hover:border-gray-800"
                id="enterpriseName"
                name="enterpriseName"
                type="text"
              ...
              />
              <p
                v-if="isError.enterpriseName"
                class="col-start-4 col-span-9 text-red-500 text-xs italic"
              >
                Veuillez inscrire le nom de l'entreprise
              </p>
            </div>
            <div class="grid grid-cols-12 mb-3">
              <label
              ...
              <select
                ...
                id="activitySector"
                name="activitySector"
                v-model="enterprise.activitySector"
              >
                <option
                  v-for="activity in activitiesSector"
                  :key="activity._id"
                  :value="activity"
                >
                  {{ activity.value }}
      ...
      <div class="bg-white page-padding">
        <!-- 1er partie -->
        <div class="presentation-body-title text-6xl mb-10">
          Courte présentation
        </div>
        <div v-if="!isEditOrCreate" class="text-2xl">
          {{ enterprise.description }}
        </div>
        <div v-else>
          <label class="hidden" for="description">Description</label>
          <textarea
            class="w-full border border-gray-400 rounded py-3 px-3 text-gray-600 text-2xl leading-tight focus:border-gray-800 hover:border-gray-800"
            id="description"
            name="description"
            v-model="enterprise.description"
            placeholder="Inscrire une description"
          />
          <p
            v-if="isError.enterpriseName"
            class="col-start-4 col-span-9 text-red-500 text-xs italic"
        ...
        <div class="presentation-body-title text-2xl my-10">
          Personne contact
        </div>
        <div v-if="!isEditOrCreate" class="text-3xl my-10 font-semibold">
          {{ contactOnView }}
        </div>
        <input
          v-else
        ...
        <div class="grid grid-cols-12 gap-8">
          <InputEntreprise
            ...
            name="address"
            :is-error="isError.address"
            label="Adresse"
            :is-edit="isEditOrCreate"
          />
          <InputEntreprise
            class="col-span-12 md:col-span-6"
          ...
          <!-- Provinces -->
          <div
            class="col-span-12 md:col-span-6 border-l-8 border-l-gray-600 px-4"
          >
            <div class="text-gray-600 mb-4 text-2xl">Province</div>
            <div>
              <div
                v-if="!isEditOrCreate && enterprise.province"
                class="text-gray-400"
              >
                {{ enterprise.province.value }}
              </div>
              <div v-else>
                <label class="hidden" for="province">Province:</label>
                <select
                  class="block w-full border border-gray-400 rounded py-3 px-3 text-gray-600 leading-tight focus:border-gray-800 hover:border-gray-800"
                  id="province"
                  name="province"
                ...
                <p v-if="isError.province" class="text-red-500 text-xs italic">
          ...
          <InputEntreprise
            class="col-span-12 md:col-span-6"
            v-model="enterprise.image"
            name="image"
            :is-error="isError.image"
            label="URL du logo"
            :is-edit="isEditOrCreate"
          />
        </div>
...
const isLoading = computed(() => {
  ...
  if (
    isLoadedProvinces.value &&
    isLoadedActivitiesSector.value &&
    isLoadedEnterprise.value
  )
    return false;
  else return true;
});
...
function onValidate(e) {
  ...
  if (!enterprise.value.name || enterprise.value.name === "")
    isError.enterpriseName = true;
  ...
  if (!enterprise.value.description || enterprise.value.description === "")
    isError.description = true;
  else isError.description = false;

  if (!enterprise.value.address || enterprise.value.address === "")
  ...
  if (!enterprise.value.city || enterprise.value.city === "")
    isError.city = true;
  ...
  if (!validatePostalCode(enterprise.value.postalCode))
    isError.postalCode = true;
...
function onWhatFor() {
  console.log(
    "Bonjour Bruno et Mathieu, je ne sais pas ce que vous vouliez avec ce bouton!"
  );
}
