<template>
    ...
    <RouterLink  class="bg-orange-500 hover:bg-orange-700 text-white font-bold py-2 px-4 border border-orange-700 rounded" to="OffredeStageDetail">Ajouter une offre de stage</RouterLink>
    ...
    <table class="w-full text-sm text-left rtl:text-right text-gray-500 dark:text-gray-400">
        ...
        <tbody>
            <tr  v-for="offre in internshipOffersListe" :key="offre._id" :offre="offre"  class="odd:bg-white odd:dark:bg-gray-900 even:bg-gray-50 even:dark:bg-gray-800 border-b dark:border-gray-700">
                ...
                <td class="px-6 py-4">
                    {{ offre.internshipType.value }}
                </td>
                <td class="px-6 py-4">
                    {{ offre.province.value }}
                </td>
                <td class="px-6 py-4">
                    {{ offre.startDate }}
                </td>
                <td class="px-6 py-4">
                    <button @click="onDetails(offre._id)"><span class="pr-2 mdi mdi-eye-circle text-blue-600 text-lg mr-4">Afficher</span></button>
                    <button><span class="pr-2 mdi mdi-file-edit text-yellow-600 text-lg mr-4">Edit</span></button>
                    <button><span class="pr-2 mdi mdi-delete-circle text-red-600 text-lg mr-4">Delete</span></button>
                </td>
            </tr>
        </tbody>
    </table>
...
const { internshipOffersListe, allInternshipOffers } = OffresService();
const offres = ref([]);
